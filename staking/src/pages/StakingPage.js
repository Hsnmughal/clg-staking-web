...
const HomePage = (props) => {
  ...
  const [userPendingRewardsState, setUserPendingRewards] = useState(0);
  const init = async () => {
    if (isReady()) {
      return;
    }
  ...
  async function stake() {
    try {
      ...
      const arg = fromExponential(actual);
      try {
        // THIS IS THE NATIVE TOKEN THAT WILL BE STAKED(LEAD TOKEN IN OUR CASE CLG), THE APPROVE FUNCTION CONTAINS THE ADDRESS OF THE STAKINGCONTRACT
        await leadToken.methods
          .approve("0x786A20fA02e4672d550BccF0BfFf118CAAE519e6", arg)
          .send({ from: accounts[0] });

        await leadStake.methods.deposit(arg).send({ from: accounts[0] });
        await leadToken?.methods.updatePool().call();
  ...
  // MAIN FUNCTION FOR USER PENDING REWARD TIP: USE THIS FOR PENDING REWARDS
  const pendingReward = async () => {
    try {
      await leadStake?.methods.pendingReward(accounts[0]).call(function (err, res) {
        if (err) {
          console.log(err)
        } else {
          setUserPendingRewards(res)
          console.log(res)
        }
      })
    } catch (e) {
  ...
  const userInfo = async () => {
    try {
      await leadStake?.methods.userInfo(accounts[0]).call(function (err, res) {
        if (err) {
          console.log("An error occured", err)
          return
        } else {
          // WILL RETURN USER STAKED AMOUNT, REWARDDEBT AND HIS PENDING REWARDS
          setUserInfo(res)
          console.log(res)
        }
      });
